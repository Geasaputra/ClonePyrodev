...
from ProjectDark.helpers.tools import get_arg, get_text, resize_media
from ProjectDark.utils.tools import add_text_img, bash
...
async def tinying(client: Client, message: Message):
    ...
    if ik.endswith(".tgs"):
        await client.download_media(reply, "dark.tgs")
        await bash("lottie_convert.py dark.tgs json.json")
        json = open("json.json", "r")
        jsn = json.read()
        jsn = jsn.replace("512", "2000")
        ("json.json", "w").write(jsn)
        await bash("lottie_convert.py json.json dark.tgs")
        file = "dark.tgs"
        os.remove("json.json")
